...
def ask_question():
    ...
    while not exit_program:
        ...
        if user_input == '':
            print()
            print(f'{yellow}Read the text{reset}'.center(100, '.'))
            playsound('level-up.mp3')

            # Start the countdown timers sequentially
            countdown_timer(3*60)  # Preparation time
            print(f'{yellow}Play the sound track...{reset}'.center(100,))
            u_input = input(
                "\t\tWhen you are done, press 'Enter' to continue answering "
                "the question.\n"
            )
            print(f'{yellow}Write your answer.....{reset}'.center(100, '.'))
            countdown_timer(20*60)  # Answer time
